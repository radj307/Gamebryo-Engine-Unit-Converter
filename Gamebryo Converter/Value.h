...
class Value {
	...
	inline Value toMetric()
	{
		...
		case TYPE::unit: // units -> meters
			return Value(TYPE::metric, _v * __CFG.getFactor_UM());
		case TYPE::imperial: // feet -> meters
			return Value(TYPE::metric, _v * __CFG.getFactor_MI());
		default:
			return *this;
		}
	}
	...
	inline Value toUnit()
	{
		...
		case TYPE::metric: // meters -> units
			return Value(TYPE::unit, _v / __CFG.getFactor_UM());
		case TYPE::imperial: // feet -> units
			return Value(TYPE::unit, _v / __CFG.getFactor_UI());
		default:
			return *this;
		}
	}
	...
	inline Value toImperial(bool useSurveyFoot = true)
	{
		...
		case TYPE::metric: // meters -> feet
			return Value(TYPE::imperial, _v / __CFG.getFactor_MI());
		case TYPE::unit: // units -> feet
			return Value(TYPE::imperial, _v * __CFG.getFactor_UI());
		default:
			return *this;
		}
	}
...
public:
	...
	inline std::string asString(bool smallUnits = false)
	{
		...
		std::stringstream ss;
		ss.precision(__CFG.getPrecision());
	...
	{
		std::cout.precision(__CFG.getPrecision());
		std::cout << std::fixed;
