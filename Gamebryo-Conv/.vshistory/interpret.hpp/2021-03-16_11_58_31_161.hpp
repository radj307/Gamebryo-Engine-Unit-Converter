...
inline void convert(Param& p)
{
	...
	if ( (p.in._t != ValType::TYPE::NONE) && (p.out._t != ValType::TYPE::NONE) && (p.val != -0.0f) ) {
		...
		else std::cout << sys::error << p.in.asString() << " to " << p.out.asString() << " isn't a valid conversion." << std::endl;//sys::msg(sys::error, p.in.asString() + " to " + p.out.asString() + " isn't a valid conversion.");
	}
	// clear params
	p.clear();
}
