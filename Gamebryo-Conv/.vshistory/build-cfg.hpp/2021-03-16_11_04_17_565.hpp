...
public:
	...
	std::string get(std::string&& header, std::string&& key) noexcept
	{
		return _config->get(std::forward<std::string>(header), std::forward<std::string>(key)).value_or("");
	}
	INI::key_list* get(std::string&& header) noexcept
	{
	 ...
	 * @param os	- Target output stream reference.
	 * @param c		- Target config reference.
	 * @returns ostream&
	 */
